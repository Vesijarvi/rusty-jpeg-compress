...
fn read_u16(reader: &mut BufReader<File>) -> u16 {
    let mut c: [u8; 2] = [0; 2];
    reader.read_exact(&mut c).expect("read_u16 fail");
...
pub fn marker_detector(mut reader: BufReader<File>) -> std::io::Result<()> {
    ...
    loop {
        ...
        reader.read(&mut c)?;
        match c[0] {
            0x00 => { /* 0xFF 後綴 0x00 並不代表 marker */ },
            0xD8 => { println!("SOI, start of image."); }
            0xD9 => { println!("EOI, end of image."); }
            m => {
                let length = read_u16(&mut reader);
                println!("marker: {:#X?}, recognized as {}, length: {}", m, marker_info(m), length);
            }
        }
        if c[0] == 0xD9 {
            break;
